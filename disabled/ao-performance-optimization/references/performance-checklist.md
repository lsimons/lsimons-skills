...
Quick reference checklist for web application performance. Use alongside the `ao-performance-optimization` skill.
...
The decision material (which layer, which invalidation strategy, what never to cache) lives in the `ao-performance-optimization` skill. This section covers the read/write patterns and the checklist.
