...
- `.claude/skills/sbp-terraform/` — generic Terraform / OpenTofu baseline (module patterns, testing, CI/CD, security). MCAF is layered on top.
- `.claude/skills/sbp-mcaf-module/` — MCAF deltas over the generic skill; structural authoring and PR review.
- `.claude/skills/sbp-review-mcaf/` — full qualitative review (best practices, usability, LCM, supply chain, security).
...
Modules drift behind Terraform, provider, and action releases. The targets below are the currency bar; anything older is technical debt to close on next touch. The `sbp-review-mcaf` skill flags LCM findings per module.
...
Terraform modules execute CI workflows and, in a few cases, bundle Lambda source. That gives two concrete supply-chain surfaces: **GitHub Actions** and **Lambda dependency manifests**. The `sbp-review-mcaf` skill flags supply-chain findings per module.
