---
name: ao-constraint-driven-development
...
Other skills in this pack describe what good looks like. `ao-code-review-and-quality` gives you five axes. `ao-test-driven-development` gives you a cycle. `ao-security-and-hardening` gives you a threat list. All of that lives in prose the agent reads and may or may not follow, and none of it survives the end of the session.
...
- You're about to run `ao-autonomous-plan-execution` or any autonomous loop, and the only thing standing between it and main is a test suite the agent also wrote
...
- The user wants a code review right now (`ao-code-review-and-quality`) or a CI pipeline built (`ao-ci-cd-and-automation`)
- Pre-product-market-fit code with a two-week expected lifetime — the floor below is still worth it, the rest isn't

## Loading Constraints

The interview needs a live user. **Don't run it in non-interactive contexts** (CI, scheduled loops, `ao-autonomous-plan-execution`). If constraints are missing and you're in one of those, apply the Floor below, note that you did, and flag the rest for a human.
...
Follow the one-question-at-a-time discipline from `ao-interview-me`, with one change: every question here has a default, so "I don't know" is a complete answer that still produces a working config.
...
| Phase | Skill | What runs | Budget |
|-------|-------|-----------|--------|
| Build | `ao-incremental-implementation` | Types, lint, secrets, the floor | under 5s, changed file only |
| Verify | `ao-test-driven-development` | Related tests, coverage on changed lines | under 90s |
| Review | `ao-code-review-and-quality` | Everything, plus the guards below | minutes |
| Ship | `ao-shipping-and-launch` | Direction checks, no regressions | CI |
...
- `ao-interview-me` — the one-question-at-a-time discipline this skill's intake borrows
- `ao-code-review-and-quality` — how to review; this skill decides what the review enforces
- `ao-ci-cd-and-automation` — building the pipeline these constraints run in
- `ao-test-driven-development` — the suite that coverage and mutation constraints measure
- `ao-security-and-hardening` — what the security dimension should contain
- `ao-performance-optimization` — where the performance numbers come from
