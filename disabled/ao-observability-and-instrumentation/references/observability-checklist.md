...
Quick reference for instrumenting production code. Use alongside the `ao-observability-and-instrumentation` skill.
...
- [ ] No secrets, tokens, passwords, or unredacted PII in any log line (hard rule from `ao-security-and-hardening`)
...
For launch-day monitoring sequence and rollback triggers, see the `ao-shipping-and-launch` skill.
