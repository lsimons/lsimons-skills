...
4. Does it have known vulnerabilities? (`pnpm audit`, or `uv run pip-audit` for Python)
...
For triaging `pnpm audit` / `pip-audit` findings and supply-chain risk (typosquatting, compromised maintainers), follow the `ao-security-and-hardening` skill — this section covers the upgrade *workflow*, that one covers the security verdict.
