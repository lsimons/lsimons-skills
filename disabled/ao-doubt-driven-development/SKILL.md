---
name: ao-doubt-driven-development
...
This is not `ao-code-review-and-quality`. That skill produces a verdict on a finished artifact. This is an in-flight posture: non-trivial decisions get cross-examined while course-correction is still cheap.
...
- **Do NOT add this skill to a subagent's own skill list.** A subagent that follows Step 3 would spawn another subagent — subagents cannot spawn subagents on most platforms.
...
In Claude Code, use a fresh subagent (e.g. the built-in `general-purpose` type) so the reviewer starts with isolated context by design.

**The adversarial prompt above takes precedence over any reviewer default response shape.** A reviewer prompted for a balanced verdict (strengths and weaknesses) will default to that shape unless overridden; doubt-driven needs issues-only output. Paste the adversarial prompt verbatim into the invocation so it overrides the default.
...
| "I'll do doubt at the end with `ao-code-review-and-quality`" | That skill is a final gate. Doubt-driven catches wrong directions early when course-correction is cheap. By PR time it's too late. |
...
- Doubting only after committing — that's `ao-code-review-and-quality`, not doubt-driven development
...
- **`ao-code-review-and-quality`**: complementary. It's a post-hoc PR verdict; doubt-driven is in-flight per-decision. Use both.
- **`ao-source-driven-development`**: SDD verifies *facts about frameworks* against official docs. Doubt-driven verifies *your reasoning about the artifact*. SDD checks the API exists; doubt-driven checks you used it correctly under the contract.
- **`ao-test-driven-development`**: TDD's RED step is doubt made concrete — a failing test is a disproof attempt. When TDD applies, that failing test *is* the doubt step for behavioral claims.
- **`ao-debugging-and-error-recovery`**: when the reviewer surfaces a real failure mode, drop into the debugging skill to localize and fix.
