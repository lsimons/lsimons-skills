...
description: Executes an entire task plan hands-off after a single approval. Use when the user asks to build, implement, or run "the whole plan," "all tasks," or "autonomously" / "auto" once a plan already exists. Not for implementing one task at a time — use incremental-implementation and test-driven-development directly for that.
---
...
3. **Stop and ask the user** (do not push through) when:
   ...
   - a task is high-risk or irreversible — auth/permission changes, destructive data migrations, payments, deletions, deploys, anything touching secrets, or anything that can't be undone with `git revert` → follow `doubt-driven-development` and get explicit sign-off before continuing
