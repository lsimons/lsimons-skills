...
Task arrives
    ...
    │   └── Whole plan, hands-off after one approval? → autonomous-plan-execution
...
- `sbp-debug-investigation` — investigate and fix bugs in mission-critical systems (instead of `debugging-and-error-recovery`)
- `sbp-dependency-audit` — audit dependencies for supply-chain and maintainability risk
- `sbp-deploy-checklist` — go/no-go production deploy checklist (instead of `shipping-and-launch`)
- `sbp-explain-codebase` — explain unfamiliar code, infrastructure, or architecture
- `sbp-feature-development` — build a feature via TDD with mission-critical rigor (instead of `spec-driven-development` + `incremental-implementation`)
- `sbp-incident-review` — blameless post-incident analysis
- `sbp-observability-check` — four-pillars observability coverage check (instead of `observability-and-instrumentation`)
- `sbp-refactor` — refactor without changing observable behavior (instead of `code-simplification`)
- `sbp-runbook-author` — generate operational runbooks
- `sbp-safe-change` — plan a high-risk production change
- `sbp-secure-code-review` — security review for mission-critical systems (instead of `security-and-hardening`)
- `sbp-test-authoring` — write tests that prove functionality and catch regressions (instead of `test-driven-development`)
...
- `terraform` — generic Terraform/OpenTofu guidance
...
7.  incremental-implementation  → Build slice by slice (or `autonomous-plan-execution` to run every task hands-off after one approval)
...
| Build | autonomous-plan-execution | Execute the whole plan hands-off after one approval |
