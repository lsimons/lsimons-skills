...
Qualitative review for Schuberg Philis MCAF Terraform modules. This is the judgement layer on top of the structural checklists in the [`sbp-terraform`](../sbp-terraform/SKILL.md) and [`sbp-mcaf-module`](../sbp-mcaf-module/SKILL.md) skills — what the eye catches that a rubric cannot:
...
The ground truth for every rule lives in [`../sbp-mcaf-module/GUIDE.md`](../sbp-mcaf-module/GUIDE.md). This skill is the playbook for applying it.
...
For small one-rule structural questions (e.g. "does this look MCAF-compliant?"), use `sbp-mcaf-module` instead — it's lighter-weight.
...
- **Explicit single path** ("review `~/code/sbp-terraform-aws-mcaf-s3`") → that directory.
...
2. **Variable design** —
   - Every `variable` has `description` + `type` in the canonical arg order (per `../sbp-mcaf-module/GUIDE.md §3`): `type` → `default` → `description` → `nullable` / `sensitive` → `validation`. `description` before `type` or `validation` before `type`/`description` are the common violations.
...
- **Do not repeat structural checklist items** unless they are the headline issue. The `sbp-mcaf-module` skill handles "missing Taskfile" etc.; this skill adds the judgement layer.
...
- Each subagent should be briefed to skim `../sbp-mcaf-module/GUIDE.md` §3/§5/§7/§11 first and reference it as source of truth.
...
- Do not duplicate the structural checklist line-by-line — that is the `sbp-mcaf-module` skill's job.
...
- [`../sbp-mcaf-module/GUIDE.md`](../sbp-mcaf-module/GUIDE.md) — the source of truth for every rule cited above.
- [`sbp-terraform`](../sbp-terraform/SKILL.md) skill — generic Terraform/OpenTofu baseline.
- [`sbp-mcaf-module`](../sbp-mcaf-module/SKILL.md) skill — structural checklist for authoring and PR review.
