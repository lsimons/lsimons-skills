...
A standing, project-wide bar that every change must clear before it counts as done. Unlike acceptance criteria, which vary per task and answer "did we build the right thing?", the Definition of Done is the same every time and answers "is this finished to our standard?". Use it as the final gate in `ao-planning-and-task-breakdown`, `ao-incremental-implementation`, and `ao-shipping-and-launch`.
...
The depth behind these items lives in `ao-code-review-and-quality` (the five-axis review) and `ao-code-simplification` (reducing complexity without changing behavior).
...
- [ ] Architectural decisions worth preserving are recorded (see `ao-documentation-and-adrs`)
- [ ] Documentation describes the current state in timeless language, not the change history

### Ship-readiness
- [ ] Security implications reviewed for any untrusted input, auth, or data handling (see `ao-security-and-hardening`)
- [ ] Observability in place for new critical paths (logs, metrics, traces) (see `ao-observability-and-instrumentation`)
- [ ] Rollback path exists for anything risky (see `ao-shipping-and-launch`)
...
- **Per release**: the full checklist is the floor; `ao-shipping-and-launch` adds the deploy-specific gates on top.
