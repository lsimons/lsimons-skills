...
### Auditing a Web App: Deep vs. Quick Mode

When the task is a standalone performance audit of a web application (not utility libraries, CLIs, or server-only code with no browser-facing output), first determine what data is actually available — don't guess at scores you can't back up.

**Deep mode** — use when any of these is available:
- A Lighthouse JSON report (e.g. `npx lighthouse <url> --output json --output-path ./report.json`, or `npx -p chrome-devtools-mcp chrome-devtools lighthouse_audit --output-format=json`)
...
- A live URL plus the `chrome-devtools` MCP server configured in the harness — capture metrics directly via `lighthouse_audit` and `performance_*` tools, see `ao-browser-testing-with-devtools`
- The Chrome DevTools MCP CLI invoked locally (`npx -p chrome-devtools-mcp chrome-devtools <tool>` or after `npm i -g chrome-devtools-mcp`)

**Quick mode** — default when none of the above are available. Scan source code for the structural anti-patterns below and label every finding `potential impact`, not a measured score.

**Never populate a scorecard with unsourced numbers.** In Quick mode, report findings as potential impact; only Deep mode's sourced data (Lighthouse/PSI/CrUX/trace) earns an actual score. Produce a ranked list of findings with file:line references and fix recommendations, positive observations, and proactive recommendations.
