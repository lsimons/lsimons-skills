---
name: claude-history
description: Search, browse, and quote prior Claude Code conversations with the claude-history CLI. Use when recovering a past decision, debugging context, error message, or implementation detail from an earlier session — including "what did we decide about X", "have I hit this error before", "find the session where we changed Y", or when asked to read or quote an old transcript.
---

<!--
Vendored from https://github.com/raine/claude-history (skills/claude-history/SKILL.md).
The upstream file ships without YAML frontmatter, so it is rejected by the
skills.sh CLI and carries no description for skill matching. The frontmatter
above is added locally; the body below is upstream verbatim. Re-sync by
re-fetching the upstream file and keeping this header.
Requires the `claude-history` binary, installed by claude/install.py.
-->
