...
- A live URL plus the `chrome-devtools` MCP server configured in the harness — capture metrics directly via `lighthouse_audit` and `performance_*` tools, see `browser-testing-with-devtools`
